		...
		public PatientDetails(Stage stage, double sizeX, double sizeY) {
			...
			EventHandler<ActionEvent> backBtnClick = new EventHandler<ActionEvent>() {
				...
				public void handle(ActionEvent e) {
					...
					PatientRecordsScene pRC = new PatientRecordsScene(stage, sizeX, sizeY);
					
					content.translateXProperty().set(scene.getHeight());
					content.getChildren().add(pRC.content);
					
					Timeline timeLine = new Timeline();
					KeyValue kv = new KeyValue(pRC.content.translateXProperty(), 0, Interpolator.EASE_IN);
